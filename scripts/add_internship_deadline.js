...
try {
  ...
  console.log("Connected to MongoDB for internship deadline migration.");

  const aprilTenthDate = new Date("2026-04-10");
  ...
  const result = await Internship.updateMany(
    ...
    {
      $set: { lastDateToApply: aprilTenthDate },
    }
  );

  console.log(`Updated ${result.modifiedCount || result.nModified || 0} internship(s) with lastDateToApply = April 10, 2026.`);
